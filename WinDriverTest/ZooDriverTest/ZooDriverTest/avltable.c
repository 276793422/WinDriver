...
NTSTATUS
NTAPI
ZOO_AVL_MATCH_FUNCTION (
						__in PZOO_GENERIC_TABLE Table,
						__in PVOID UserData,
						__in PVOID MatchData
						);
typedef ZOO_AVL_MATCH_FUNCTION *PZOO_AVL_MATCH_FUNCTION;

static
NTSTATUS
MatchAll (
		  IN PZOO_GENERIC_TABLE Table,
		  IN PVOID P1,
		  IN PVOID P2
		  );
...
VOID
ZooInitializeGenericTable (
						   IN PZOO_GENERIC_TABLE Table,
						   IN PZOO_AVL_COMPARE_ROUTINE CompareRoutine,
						   IN PZOO_AVL_ALLOCATE_ROUTINE AllocateRoutine,
						   IN PZOO_AVL_FREE_ROUTINE FreeRoutine,
						   IN PVOID TableContext
						   );


PVOID
ZooInsertElementGenericTable (
							  IN PZOO_GENERIC_TABLE Table,
							  IN PVOID Buffer,
							  IN CLONG BufferSize,
...
PVOID
ZooInsertElementGenericTableFullAvl (
									 IN PZOO_GENERIC_TABLE Table,
									 IN PVOID Buffer,
									 IN CLONG BufferSize,
									 OUT PBOOLEAN NewElement OPTIONAL,
...
PVOID
NTAPI
ZooLookupElementGenericTableFullAvl (
									 PZOO_GENERIC_TABLE Table,
									 PVOID Buffer,
...
BOOLEAN
ZooIsGenericTableEmpty (
						IN PZOO_GENERIC_TABLE Table
						);


PVOID
ZooGetElementGenericTableAvl (
							  IN PZOO_GENERIC_TABLE Table,
							  IN ULONG I
							  );
...
ULONG
ZooNumberGenericTableElements (
							   IN PZOO_GENERIC_TABLE Table
							   );


PVOID
ZooEnumerateGenericTableWithoutSplayingAvl (
											IN PZOO_GENERIC_TABLE Table,
											IN PVOID *RestartKey
											);
...
PVOID
NTAPI
ZooEnumerateGenericTableLikeADirectory (
										IN PZOO_GENERIC_TABLE Table,
										IN PZOO_AVL_MATCH_FUNCTION MatchFunction OPTIONAL,
...

{
    ...
    if (ZooIsGenericTableEmpty(Table)) {
...

{
    ...
    RtlZeroMemory( Table, sizeof(ZOO_GENERIC_TABLE) );
...

{
    ...
    return ZooInsertElementGenericTableFullAvl(
                Table,
                Buffer,
                BufferSize,
                NewElement,
...

{
    ...
    return ZooLookupElementGenericTableFullAvl(
                Table,
                Buffer,
                &NodeOrParent,
                &Lookup
...

{
    ...
    return ZooEnumerateGenericTableWithoutSplayingAvl( Table, &Table->RestartKey );
}
